...
# halt = np.load('./Documents/Height profiles/ice_block_0_5.npy')
# d = 0.37566904425667014 #0_5
# halt = np.load('./Documents/Height profiles/ice_block_0_23.npy')
# d = 0.3957774907299554 #0_23
halt = np.load('./Documents/Height profiles/ice_block_0_15.npy')
d = 0.39250081308793844 #0_15
# halt = np.load('./Documents/Height profiles/ice_block_0_8(1).npy')
# d = 0.39250081308793844 #0_8(1)
# halt = np.load('./Documents/Height profiles/ice_block_0_8(2).npy')
# d = 0.388995700793867 #0_8(2) #mejor no usar

halg = nangauss(halt, 2)
# halg = gaussian(halt,5)
...
t1 = time()  
mks = ~np.isnan(halg) 
# # mkse = mks*1. + binary_erosion(mks,disk(50))
mkse = binary_erosion( np.pad(mks, ((0,0),(1,1),(1,1))), np.ones((1,100,100)) )[:,1:-1,1:-1] #+ mks*1.
...
for n in tqdm(range(nt)):
    coeff, r, rank, s = polyfit(n,halg,mmmm,xr,yr)
    cuapla = pol2(coeff,xr,yr) 
    difes.append( (halg[n]-cuapla)*mmmm[n] )
...
for n in range(0,nt,4):
    # plt.plot(y,halt[n,:,400])
    plt.plot(y,halg[n,:,400])
plt.show()

#%%
# mee = np.nanmean(halg, axis=(1,2))
# meme = np.nanmean(melt, axis=(1,2))
megt = np.nanmean(gt, axis=(1,2))
sdgt = np.nanstd(gt, axis=(1,2))

plt.figure()
# plt.plot(t/60,mee,'.-')
# plt.plot(t[:-1]/60,meme,'.-')
# plt.plot(t/60,megt*30,'.-')
plt.errorbar( t/60, megt, yerr=sdgt, fmt='ks', capsize=2 )
plt.grid()
plt.show()
...
# plt.scatter( gy[:,:,:] ,  gt[:,:,:])
# plt.grid()
# plt.xlabel('y gradient')
# plt.ylabel('t gradient')
# plt.show()
...
plt.plot( gt[n,:,500] *30 + 0.2, 'b-' )
# plt.plot( gy[n,:,500] , 'r-'  )
# plt.plot( gy[n,:,500], gt[n,:,500] , 'r-'  )

plt.plot( melt[n,:,500]*1 + 0.3)
# plt.plot( dey[n,:,500]*1, 'r-.' )
plt.plot( halg[n,:,500]/20+2, 'g--' )
plt.grid()
plt.show()

plt.figure()
for k in range(400,600,20):
    # plt.scatter(dey[n,100:650,k], melt[n,100:650,k])
    plt.scatter(gy[n,100:650,k], gt[n,100:650,k])
plt.grid()
plt.show()
plt.figure()
for k in range(200,500,20):
    # plt.scatter(dey[n,100:650,k], melt[n,100:650,k])
    plt.scatter(gx[n,k,370:630], gt[n,k,370:630])
plt.grid()
plt.show()

deyma = ma.masked_invalid(dey[n,100:650,370:630])
meltma = ma.masked_invalid(melt[n,100:650,370:630])
print( ma.corrcoef( deyma.flatten(), meltma.flatten() ))

gyma = ma.masked_invalid(gy[n,100:650,370:630])
gxma = ma.masked_invalid(gx[n,100:650,370:630])
gtma = ma.masked_invalid(gt[n,100:650,370:630])
print( ma.corrcoef( gyma.flatten(), gtma.flatten() ))
print( ma.corrcoef( gxma.flatten(), gtma.flatten() ))
...
gymas = ma.masked_invalid( gy * mmmm )
gxmas = ma.masked_invalid( gx * mmmm )
gtmas = ma.masked_invalid( gt * mmmm )
...
plt.figure()
plt.plot(t/60 ,coey, label='dh/dy')
plt.plot(t/60, coex, label='dh/dx')
plt.xlabel('time (min)')
plt.ylabel('Correlation')
plt.legend()
plt.grid()
# plt.savefig('./Documents/corr_8.png',dpi=400, bbox_inches='tight')
plt.show()
...
plt.show()
# plt.figure()
# plt.scatter( gxmas[n], gymas[n], s=1 )
# plt.xlabel('dh/dx')
# plt.ylabel('dh/dy')
# plt.grid()
# # plt.xlim([-0.58,0.58])
...
ax1.plot(y, gy[n,:,500], color=color)
...
ax2.plot(y, gt[n,:,500] * 60, color=color)
...
for n in tqdm(range(nt)):
    coeff, r, rank, s = polyfit(n,halg,mmmm,xr,yr)
    cuads.append( pol2(coeff,xr,yr) )
cuads = np.array(cuads)

mee = np.nanmean(halg, axis=(1,2))
sde = np.nanstd(halg, axis=(1,2))

sdef = np.nanstd((halg-cuads)*mmmm,axis=(1,2))
...
ax[0].grid()
ax[1].grid()
ax[0].set_ylabel('vel y (mm/min)')
ax[1].set_ylabel('vel x (mm/min)')
ax[1].set_xlabel('scallop label')
# plt.savefig('./Documents/vels_23.png',dpi=400, bbox_inches='tight')
plt.show()
...
corn = []
for n in tqdm(range(nt)):
    difep = np.pad( difes[n], ((300,300),(0,0)) )
    difep[ np.isnan(difep)] = 0
    
    cors2, cors = [],[]
    for r in range(700):
        cor = difep * np.roll(difep, r,axis=0)
        # cors2.append( np.nanmean(cor**2) )
        cors.append( np.nanmean(cor) )
...
# plt.show()
# plt.figure()
# plt.imshow(np.roll(difep, 580,axis=0))
# plt.show()
...
plt.figure()
for n in range(50,65,1):
    pek = find_peaks(corn[n][:120],distance=5)[0]
    
    plt.plot( r*d, corn[n]/corn[n][0], label=n/2 )
    if len(pek)>0: plt.plot(r[pek]*d, (corn[n]/corn[n][0])[pek],'k.' )
    if len(pek)>0:
        lams.append(r[pek[0]]*d)
        ns.append(n)
        cl.append((corn[n]/corn[n][0])[pek[0]])
ns = np.array(ns)
...
n = 40
hannan = difes[n].copy()
hannan[ np.isnan(difes[n]) ] = 0

plt.figure()
# plt.imshow(hannan)
plt.plot(hannan[275:865,400])
plt.show()

# ftf = np.fft.fft2( hannan )
# ftfs = np.fft.fftshift( ftf )

ftf = np.fft.fft( hannan[275:865,400] )
ftfs = np.fft.fftshift( ftf )
ky = np.fft.fftshift( np.fft.fftfreq(865-275, d=y[1]-y[0]) )

plt.figure()
# plt.imshow( np.log(np.abs(ftfs))  )
plt.plot(ky, np.log(np.abs(ftfs))  )
plt.show()

#%%
# with distance between max
n = 0
...
plt.figure()
plt.errorbar(t/60, lmea, yerr=lsd, capsize=2, fmt='ks')
plt.grid()
...
bord = [200,700,360,480]
# bord = [340,830,340,450]

lmea_i,lmed_i,lsd_i = [],[],[]
for n in range(nt):
    lons =[]
    for l in range(bord[3]-bord[2]):
        line = (difes[n,bord[0]:bord[1],bord[2]:bord[3]])[:,l]
        pek = find_peaks(line, prominence=0.5)[0]
        long = y[pek][:-1] - y[pek][1:]
        lons += list(long) 
    lmea_i.append( np.mean(lons) )
    lmed_i.append( np.median(lons) )
    lsd_i.append( np.std(lons) )


plt.figure()
plt.errorbar(t/60, lmea_i, yerr=lsd_i, capsize=2, fmt='ks')
plt.errorbar(t/60, lmea, yerr=lsd, capsize=2, fmt='bs')
plt.grid()
plt.ylabel('Wavelenght (mm)')
plt.xlabel('Time (min)')
# plt.savefig('./Documents/wales_8.png',dpi=400, bbox_inches='tight')
plt.show()
#%%
# bord = [340,830,340,450]
# plt.figure()
# plt.imshow(difes[-2,bord[0]:bord[1],bord[2]:bord[3]])
# # plt.imshow(difes[-2])
# plt.show()
#%%
lmea,lmed,lsd = [],[],[]
...
#%%
# np.seterr(all='ignore')
halt5 = np.load('./Documents/Height profiles/ice_block_0_5.npy')
# d = 0.37566904425667014 #0_5
halt23 = np.load('./Documents/Height profiles/ice_block_0_23.npy')
# d = 0.3957774907299554 #0_23
halt15 = np.load('./Documents/Height profiles/ice_block_0_15.npy')
# d = 0.39250081308793844 #0_15

me5 = np.nanmean(halt5, axis=(1,2))
me15 = np.nanmean(halt15, axis=(1,2))
me23 = np.nanmean(halt23, axis=(1,2))

plt.figure()
plt.plot(me5 - me5[0],'b.-')
plt.plot(me15 - me15[0],'g.-')
plt.plot(me23 - me23[0],'r.-')
plt.grid()
plt.show()
